"""
LaTeX -> PDF/PNG renderer.

Compiles a LaTeX string with ``latexmk``, falling back through
``pdflatex`` -> ``lualatex`` -> ``xelatex``, then crops whitespace and
converts the result to a PNG. Returns both the cropped PDF and PNG as
in-memory bytes.

System dependencies: ``latexmk``, a LaTeX engine, ``poppler``
(``pdftoppm`` / ``pdfinfo``), and ``ghostscript``.
"""
...
def tex2png(
    ...
    """Convenience: same as :func:`tex2img` but returns just the PNG bytes."""
    return tex2img(
        code, size=size, timeout=timeout,
        expand_to_square=expand_to_square, verbose=verbose,
    )["image"]
