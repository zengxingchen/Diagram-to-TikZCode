...
class MSE_withoutcanny:
    """
    Plain pixel MSE distance (score = l2 / 2), kept compatible with the
    ...
    - The raw score is clipped to ``[0, 2]``; when ``as_percent=True``
    ...
    def __init__(
        ...
        blur_ksize: int = 13,
        eps_std: float = 1e-6,
        clip_score: Tuple[float, float] = (0.0, 2.0),
    ...
    def reconstruction_reward(self, rendered_img: np.ndarray, ground_truth_img: Dict[str, Any]) -> Optional[float]:
        ...
        - z-score
        - l2 = mean((pred - gt) ** 2);   score = l2 / 2
        - clip to ``[0, 2]``;  if ``as_percent=True`` multiply by 100.
        """
        try:
